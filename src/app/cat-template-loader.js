...
export async function loadCatTemplates() {
  ...
  const svg = document.querySelector("svg"),
    [x, y, width, height] = (
      ...
      .split(/\s+/)
      .map(Number);
  const templateId = svg.getAttribute("data-template-id"),
    zones = {};
  for (const node of svg.querySelectorAll("polygon[data-zone]")) {
    const zoneId = node.getAttribute("data-zone");
    zones[zoneId] ??= {
      zoneId,
      layer: Number(node.getAttribute("data-layer")),
      tiePriority: Number(node.getAttribute("data-tie-priority")),
      polygons: [],
    };
  ...
  return {
    [templateId]: {
      ...
      silhouette: parsePoints(
        svg.querySelector("polygon[data-silhouette]").getAttribute("points"),
      ),
      zones,
    },
  };
