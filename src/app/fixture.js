...
export function createFixtureWorld(templates = {}) {
  ...
  const dispatch = (command) => { const result = applyCommand(world, command); if (!result.ok) throw new Error(`Fixture: ${result.error.code}`); world = result.world; };
  dispatch({ type: "createDrawing", drawing: { id: "cup-source", width: 140, height: 120, background: "transparent", strokes: [
    { id: "cup-body", tool: "brush", color: "#d76f52", width: 52, points: [{ x: 52, y: 38, pressure: .5 }, { x: 52, y: 78, pressure: .5 }] },
    { id: "cup-handle", tool: "brush", color: "#d76f52", width: 14, points: [{ x: 80, y: 43, pressure: .5 }, { x: 108, y: 43, pressure: .5 }, { x: 108, y: 67, pressure: .5 }, { x: 82, y: 67, pressure: .5 }] },
  ] } });
  dispatch({ type: "createCutout", entityId: "cup", newDrawingId: "cup-art", sourceDrawingId: "cup-source", contour: [{ x: 20, y: 18 }, { x: 82, y: 18 }, { x: 82, y: 31 }, { x: 111, y: 31 }, { x: 122, y: 42 }, { x: 122, y: 68 }, { x: 111, y: 79 }, { x: 82, y: 79 }, { x: 76, y: 98 }, { x: 27, y: 98 }, { x: 20, y: 18 }], targetSurfaceId: "table", worldPosition: { x: 835, y: 300 }, label: "Чашка" });
  dispatch({ type: "createNotebook", notebookId: "notebook-home-a", label: "Дом у окна", coverSurfaceId: "notebook-a-cover", coverDrawingId: "notebook-a-cover-art", spreads: [1, 2, 3].map((n) => ({ id: `notebook-a-spread-${n}`, surfaceId: `notebook-a-room-${n}`, drawingId: `notebook-a-room-${n}-art` })), targetSurfaceId: "table", transform: t(70, 610, -.025), width: 390, height: 240, state: "closed" });
  dispatch({ type: "createNotebook", notebookId: "notebook-home-b", label: "Дом у сада", coverSurfaceId: "notebook-b-cover", coverDrawingId: "notebook-b-cover-art", spreads: [1, 2, 3].map((n) => ({ id: `notebook-b-spread-${n}`, surfaceId: `notebook-b-room-${n}`, drawingId: `notebook-b-room-${n}-art` })), targetSurfaceId: "table", transform: t(930, 610, .02), width: 390, height: 240, state: "closed" });
  dispatch({ type: "createSheet", sheetId: "sheet-car", label: "Лист-машина", insideSurfaceId: "sheet-car-inside", outerTopSurfaceId: "sheet-car-top", outerBottomSurfaceId: "sheet-car-bottom", insideDrawingId: "sheet-car-inside-art", outerTopDrawingId: "sheet-car-top-art", outerBottomDrawingId: "sheet-car-bottom-art", targetSurfaceId: "table", transform: t(505, 650, -.04), width: 330, height: 205, state: "open" });
  dispatch({ type: "createSheet", sheetId: "sheet-nested", label: "Вложенный лист", insideSurfaceId: "sheet-nested-inside", outerTopSurfaceId: "sheet-nested-top", outerBottomSurfaceId: "sheet-nested-bottom", insideDrawingId: "sheet-nested-inside-art", outerTopDrawingId: "sheet-nested-top-art", outerBottomDrawingId: "sheet-nested-bottom-art", targetSurfaceId: "sheet-car-inside", transform: t(55, 45, .03, .62), width: 250, height: 150, state: "closed" });
  dispatch({ type: "setSheetState", sheetId: "sheet-car", state: "closed" });
