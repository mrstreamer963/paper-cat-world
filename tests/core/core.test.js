...
const entity = (id, surfaceId, transform = t(10, 10), zIndex = 0) => ({
  id,
  kind: "paper",
  surfaceId,
  transform,
...
const surface = (id, hostEntityId, transform = t(0, 0)) => ({
  id,
  kind: "generic",
  hostEntityId,
  transform,
...
function nestedWorld() {
  let world = createWorld({
    table: { width: 500, height: 500, surfaceId: "table" },
  });
  world = succeed(world, {
    type: "createEntity",
    entity: entity("box", "table", t(100, 80, 0.2, 1.2)),
    surfaces: [surface("inside", "box", t(5, 7, -0.1, 0.8))],
  });
  world = succeed(world, {
    type: "createEntity",
    entity: entity("cat", "inside", t(20, 30, 0.3, 0.7)),
  });
  return world;
}
...
describe("world and commands", () => {
  ...
  it("moves locally and preserves a requested world pose", () => {
    ...
    world = succeed(world, {
      type: "createEntity",
      entity: entity("target", "table", t(250, 100, -0.3, 1.1)),
      surfaces: [surface("target-inside", "target", t(2, 3))],
    });
    world = succeed(world, {
      type: "moveEntity",
      entityId: "cat",
    ...
    world = succeed(world, {
      ...
      entityId: "cat",
      targetSurfaceId: "target-inside",
      transform: desired,
    });
    expect(getEntityWorldTransform(world, "cat")).toEqual(
      expect.objectContaining({
        x: expect.closeTo(280, 8),
        y: expect.closeTo(150, 8),
        rotation: expect.closeTo(0.5, 8),
        scale: expect.closeTo(0.75, 8),
      }),
    );
  });
  ...
  it("propagates visibility and rejects hidden targets", () => {
    let world = nestedWorld();
    world = succeed(world, {
      type: "setSurfaceVisibility",
      surfaceId: "inside",
      visibility: "hidden",
    });
    expect(isEntityVisible(world, "cat")).toBe(false);
    const rejected = applyCommand(world, {
      type: "moveEntity",
      entityId: "cat",
      targetSurfaceId: "inside",
      transform: t(30, 30),
    });
  ...
  it("rejects direct and indirect cycles atomically", () => {
    ...
    world = succeed(world, {
      type: "createEntity",
      entity: entity("small-box", "inside", t(40, 40)),
      surfaces: [surface("small-inside", "small-box")],
    });
  ...
  it("returns stable errors for invalid data", () => {
    ...
    const cases = [
      ...
      [
        { type: "createEntity", entity: entity("bad", "missing") },
        "SURFACE_NOT_FOUND",
      ],
      [
        {
          type: "createEntity",
          entity: entity("bad", "table", t(Number.NaN, 1)),
        },
        "INVALID_NUMBER",
      ],
      [
        { type: "createEntity", entity: entity("bad", "table", t(200, 1)) },
        "OUTSIDE_PLACEMENT_AREA",
      ],
    ];
    for (const [command, code] of cases)
      expect(applyCommand(world, command).error.code).toBe(code);
  });

  it("distinguishes unexpected programming errors from invalid references", () => {
    ...
    const result = applyCommand(createWorld(), command);
  ...
  it("raises z-index only among siblings", () => {
    ...
    world = succeed(world, {
      type: "createEntity",
      entity: entity("peer", "inside", t(60, 60), 10),
    });
  ...
  it("handles hundreds of nesting levels iteratively", () => {
    ...
    for (let index = 0; index < depth; index += 1) {
      ...
      surfaces[surfaceId] = {
        ...surface(surfaceId, entityId),
        placementArea: rect(10, 10),
      };
  ...
  it("collects multiple validation failures", () => {
    ...
    const invalid = {
      ...
      entities: {
        broken: {
          ...entity("broken", "missing", t(Number.NaN, 0)),
          zIndex: Infinity,
        },
      },
    };
